...
def dataFreqSweep(dataframe):
    (freq,
     gPrime,
     gDouble) = (columnRead(dataframe, 'f in Hz'),
                 columnRead(dataframe, "G' in Pa"),
                 columnRead(dataframe, "G'' in Pa"))
    freq, gPrime, gDouble = freq[freq > 0], gPrime[gPrime > 0], gDouble[gDouble > 0]
...
filePath = ('C:/Users/petrus.kirsten/PycharmProjects/RheometerPlots/data/031024/5pct/'
            'FreqSweep.xlsx')  # -> CEBB PC
...
mainPlot(dataPath=filePath, color='coral')
